<?php
	include_once 'inc/header.php';
?>
	<h1>Sửa Thông Tin Người Dùng</h1>
	<form action="#">
		<div class="change_img">
			<img src="../../img/per1.jpg" alt="userAvatar" id="user_pic">
			<label for="input_file" class="change_button">Thay đổi</label>
			<input type="file" id="input_file" accept="image/*">
		</div>
		<div class="edit"> <!--phan loai user-->
			<label for="loaiUser">Loại người dùng</label>
			<select name="loaiUser" id="loaiUser">
				<option value="admin">Admin</option>
				<option value="client">Khách hàng</option>
			</select>
		</div>
		<div class="edit">
			<label for="user_name">Tên</label>
			<input type="text" name="user_name" value="Nguyễn Mạnh Hùng">
		</div>
		<div class="edit">
			<label for="email">Email</label>
			<input type="text" name="email" value="manhhung271193@gmail.com">
		</div>
		<div class="buttons">
			<a href="userAdmin.php"><i class="fa-solid fa-x"></i>Đóng</a>
			<a onclick="warning()"><i class="fa-solid fa-download"></i>Cập nhật</a>
		</div>
	</form>
<?php
	include_once 'inc/footer.php';
?>           